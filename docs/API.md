...
| `--api-key <secret>` | *(empty = auth off)* | Requires the `x-api-key` header on agent/operator endpoints (`/logs`, `/collect`, `/beacon`, `/task/:id`, `/shell/:id`, `/admin/*`). `/` and `/health` stay public for monitoring. Comparison is constant-time (SHA-256 + fold). A missing/wrong key answers `401 UNAUTHORIZED`. |
