...
impl Tournament {
    ...
    pub fn generate_competitors(&self, config: &TournamentConfig) -> Vec<Competitor> {
        ...
        let archetypes = [
            ...
            (
                "Experimental",
                vec![
                    ("marl_policy", "exploratory"),
                    ("evasion", "smoke_signals"),
                    ("lateral", "dns_tunnel"),
                ],
            ),
            (
        ...
        for (name, policies) in archetypes
            .iter()
            .take(config.competitors.min(archetypes.len()))
        {
    ...
    pub fn select_runner_up(&self, competitors: &[Competitor], winner_id: Uuid) -> Option<Uuid> {
        let mut sorted: Vec<&Competitor> = competitors
            .iter()
    ...
    pub fn crossover(&self, parent_a: &Competitor, parent_b: &Competitor) -> Competitor {
        ...
        let all_keys: Vec<&String> = parent_a
            .policies
            .keys()
    ...
    pub fn mutate_next_generation(
        &self,
        winner: &Competitor,
        runner_up: Option<&Competitor>,
        generation: usize,
        ...
        for i in 1..config.competitors {
            ...
            for _ in 0..mutation_count {
                ...
                if let Some(val) = clone.policies.get(key) {
                    clone
                        .policies
                        .insert(key.clone(), format!("{}{}", val, suffix));
                }
            }
    ...
    pub fn run_tournament(&self, config: &TournamentConfig) -> TournamentResult {
        ...
        for gen in 0..config.generations {
            ...
            let runner_up_id = winner_id.and_then(|w| self.select_runner_up(&competitors, w));

            if gen + 1 < config.generations {
                if let Some(w_id) = winner_id {
                    let winner = competitors.iter().find(|c| c.id == w_id).cloned();
                    let runner_up = runner_up_id
                        .and_then(|ru_id| competitors.iter().find(|c| c.id == ru_id).cloned());

                    if let Some(w) = winner {
                        let next_gen =
                            self.mutate_next_generation(&w, runner_up.as_ref(), gen + 1, config);
            ...
            if gen + 1 == config.generations {
                let scores: Vec<(Uuid, f32)> =
                    competitors.iter().map(|c| (c.id, c.score)).collect();
